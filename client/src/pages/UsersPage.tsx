...
export function UsersPage() {
  ...
  useEffect(() => {
    console.log('Filtering users - Search:', searchQuery, 'Role:', roleFilter, 'Total users:', users.length);
    let filtered = [...users]; // Create a copy to avoid mutation

    // Filter by search query (trim whitespace and make case-insensitive)
    if (searchQuery.trim()) {
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-10 py-3 rounded-lg border border-light-border dark:border-dark-border bg-light-background dark:bg-dark-background focus:outline-none focus:ring-2 focus:ring-saffron"
                  />
                  {searchQuery && (
                    <button
                      onClick={() => setSearchQuery('')}
                      className="absolute right-3 top-1/2 transform -translate-y-1/2 text-light-muted-foreground dark:text-dark-muted-foreground hover:text-red-500 transition-colors"
                      title="Clear search"
                    >
              ...
              {filteredUsers.length === 0 && (
                <div className="text-center py-12">
                  ...
                  {searchQuery || roleFilter !== 'all' ? (
                    <>
                      <p className="text-light-muted-foreground dark:text-dark-muted-foreground font-semibold mb-2">
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-saffron hover:bg-saffron-dark text-white rounded-lg transition-colors"
                      >
                        Clear Filters
                      </button>
                    </>
