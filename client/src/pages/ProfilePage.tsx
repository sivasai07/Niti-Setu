...
import { User, Phone, MapPin, Save, X, Upload, Link as LinkIcon, Edit2, Trash2, CreditCard } from 'lucide-react';
...
export function ProfilePage() {
  ...
  const handleSave = async () => {
    ...
    if (formData.aadhar && (formData.aadhar.length !== 12 || !/^\d+$/.test(formData.aadhar))) {
      setErrors({ aadhar: 'Aadhar must be exactly 12 digits' });
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        } else if (data.field === 'aadhar') {
          setErrors({ aadhar: data.message });
