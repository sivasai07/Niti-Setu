...
export const register = async (req, res) => {
  try {
    const { name, username, phoneNumber, aadhar, city, district, state, pincode, language, pin, role } = req.body;

    // Validate aadhar is provided
    if (!aadhar || !/^[0-9]{12}$/.test(aadhar)) {
      return res.status(400).json({
        success: false,
        message: 'Aadhar is required and must be 12 digits',
        field: 'aadhar',
      });
    }
...
export const updateProfile = async (req, res) => {
  try {
    const { name, username, aadhar, city, district, state, pincode, language, profilePicture, newPin, currentPin } = req.body;
    ...
    // Check if aadhar is being changed and if it's already taken
